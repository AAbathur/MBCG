...
class UnifyConfig(OFAConfig):
    ...
    image_text_data: Optional[str] = field(
        default=None,
        metadata={"help": "image-text pair data"},
    )
    image_text_selected_cols: Optional[str] = field(
        default=None,
        metadata={"help": "image-text pair data selected cols"},
    )
    ...
    language: Optional[str] = field(
        default=None,
        metadata={"help": "language of data samples"},
    )
...
class UnifyTask(OFATask):
    def __init__(self, cfg: UnifyConfig, src_dict, tgt_dict):
        ...
        with open(os.path.join(self.cfg.neg_sample_dir, "all_comments.txt"), "r") as f1:
            for row in f1:
                cont = row.strip().split("\t")
                self.all_cmt_list.append(cont[0])
        ...
        slice_id = torch.distributed.get_rank()
        ...
        if self.cfg.image_text_data is not None:
            self.image_text_dataset = FileDataset(self.cfg.image_text_data, self.cfg.image_text_selected_cols)

        paths = []
        for fn in os.listdir(self.cfg.data):
            if self.cfg.language == "en" and fn.endswith(".tsv"):
                paths.append(os.path.join(self.cfg.data, fn))
        paths.sort()
        self.paths = shuffle(paths, random_state=17)

    def load_dataset(self, split, epoch=1, combine=False, **kwargs):        
        file_path = self.paths[(epoch - 1) % (len(self.paths))]
        ...
        self.datasets[split] = UnifyDataset(
            ...
            all_cmt_list=self.all_cmt_list,

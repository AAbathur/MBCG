...
def collate(samples, pad_idx, eos_idx, multimodal=False):
    if len(samples) == 0:
        return {}
    ...
    def merge_v2(key):
        ...
        for s in samples:
            if isinstance(s[key], list):
                cont += s[key]
                num_record.append(len(s[key]))
            else:
                cont.append(s[key])
                num_record.append(1)
        return data_utils.collate_tokens(cont, pad_idx, eos_idx=eos_idx), num_record

    id = np.array([s["id"][:30] for s in samples])
    ...
    if samples[0].get("target", None) is not None:
        target, num_record1 = merge_v2("target")
        tgt_lengths = target.ne(pad_idx).long().sum()
        ntokens = tgt_lengths.sum().item()

        if samples[0].get("prev_output_tokens", None) is not None:
            prev_output_tokens, _ = merge_v2("prev_output_tokens")
    else:
        ntokens = src_lengths.sum().item()
    conf = []
    for i, nn in enumerate(num_record1):
        cf = samples[i]['conf']
        for _ in range(nn):
            conf.append(cf)
...
class UnifyDataset(OFADataset):
    def __init__(
        ...
        ### all cmt list
        self.all_cmt_list = all_cmt_list
        ...
        self.patch_crop_transform = transforms.Compose([
            T.RandomResize(scales, max_size=672),
            transforms.CenterCrop(256),## 这两行源码没有，所以存在image size不同的问题
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
        ])
        # for detection
        ...
        ### self.bos: 0, self.eos: 2, self.src_dict.pad(): 1
    ...
    def check_and_select_neg_cmt(self, cmts, neg_cmts):
        clen = len(cmts[0].split(" "))
        abs_lens = []
        for i, nc in enumerate(neg_cmts):
            if nc in cmts:
                return False, None
            nc_len = len(nc.split(" "))
            abs_lens.append((i, abs(nc_len - clen)))
        abs_lens.sort(key=lambda x: x[1])
        select = neg_cmts[abs_lens[0][0]]
        return True, select

    def get_negative_cmt(self, cmts):
        while True:
            neg_cmts = random.sample(self.all_cmt_list, 10)
            Flag, select = self.check_and_select_neg_cmt(cmts, neg_cmts) 
            if Flag == True:
                break
        return select
    

    def process_cmt_data(self, cont):
        uniq_id, image, caption, question, refs, gt_objects, dataset_name, type = cont
        question = question[:256]

        if isinstance(image, str):
            num_pad = 4 - (len(image) % 4)
            image += "=" * num_pad
        ...
        question = 'post: {}'.format(question)
        question = self.pre_question(question, self.max_src_length+10)
        neg_cmt = self.pre_caption(self.get_negative_cmt(cmts[:]), self.max_tgt_length)

        src_item = self.encode_text(" {}".format(question), length=self.max_src_length)

        src_item = torch.cat([self.bos_item, src_item, self.eos_item])
        if len(cmts) > 3:
            cmts = cmts[:3]

        cmts = [self.pre_caption(cmt, self.max_tgt_length) for cmt in cmts]
        cmt_itm_prompt = ' what is the response to content " {} " . Is " {} "?'
        pos_src_item = self.encode_text(cmt_itm_prompt.format(question, cmts[0]), self.max_src_length+self.max_tgt_length+10)
        neg_src_item = self.encode_text(cmt_itm_prompt.format(question, neg_cmt), self.max_src_length+self.max_tgt_length+10)

        conf = 1.0
        conf = torch.tensor([conf])

        tgt_list, prev_output_list = [], []
        for j, cmt in enumerate(cmts):
            tgt_item = self.encode_text("{}".format(cmt), self.max_tgt_length)
            tgt_list.append(torch.cat([tgt_item, self.eos_item]))
            prev_output_list.append(torch.cat([self.bos_item, tgt_item]))
        ...
        example =  {
            ...
            "target": tgt_list,
            "prev_output_tokens": prev_output_list,
            "conf": conf,
            "tgt_num": len(cmts),
        }
    ...
    def process_image_text_pair(self, cont):

        uniq_id, img, caption, question, refs, gt_objects, dataset_name, type = cont
        ...
        if type == 'caption':
            ...
            tgt_item = self.encode_text(" {}".format(tgt_caption), self.max_tgt_length)
            pos_src_item = self.encode_text(' does the image describe " {} "?'.format(pos_src_caption), self.max_src_length+20)
            neg_src_item = self.encode_text(' does the image describe " {} "?'.format(neg_src_caption), self.max_src_length+20)
            ...
            tgt_item = self.encode_text(" {}".format(answer), self.max_tgt_length)
            conf = torch.tensor([conf])

            cmt_itm_prompt = ' what is the response to content " {} " . Is " {} "?'
            pos_src_item = self.encode_text(cmt_itm_prompt.format(question, answer), self.max_src_length+self.max_tgt_length+10)
            neg_src_item = self.encode_text(cmt_itm_prompt.format(question, self.get_negative_answer(answer, conf)), self.max_src_length+self.max_tgt_length+10)
            ...
            src_item = self.encode_text(' which region does the text " {} " describe?'.format(src_caption), self.max_src_length+30)
        ...
        if type == 'visual_grounding':
            ...
            region_tgt_item = self.encode_text(' {}'.format(self.pre_caption(caption, self.max_tgt_length)), self.max_tgt_length)
    ...
    def __getitem__(self, index):
        ...
        with data_utils.numpy_seed(self.seed, self.epoch):
            uniq_id, vpath, caption, question, refs, gt_objects, dataset_name, type = self.dataset[index]
            cmt_cont = [uniq_id, vpath, caption, question, refs, gt_objects, dataset_name, type]
            samples = self.process_cmt_data(cmt_cont)

            if self.split == "train" and self.dataset.data_cnt % 2 == 0:
                uniq_id, vpath, caption, question, refs, gt_objects, dataset_name, type = self.image_text_dataset[0]
                image_text_cont = [uniq_id, vpath, caption, question, refs, gt_objects, dataset_name, type]
                samples += self.process_image_text_pair(image_text_cont)

            extra_samples = []
            if self.split == 'train' and self.dataset.data_cnt % 16 == 0:
                extra_samples += self.process_pure_text(0) 
                extra_samples += self.process_pure_image(0) 
                extra_samples += self.process_detection(0)
    ...
    def collater(self, samples, pad_to_length=None):
        ...
        if samples_v2 != []:
            res_v1 = collate(samples_v1, pad_idx=self.src_dict.pad(), eos_idx=self.eos, multimodal=True)
            res_v2 = collate(samples_v2, pad_idx=self.src_dict.pad(), eos_idx=self.eos, multimodal=False)
